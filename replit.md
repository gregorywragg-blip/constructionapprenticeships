...
- **Supportive Services Redesign**: Completely redesigned the Support Services page (/resources) with embedded PDF viewers for 13 barrier resources. Users can click any barrier category card (Car Ownership, Quit Cannabis, Driver's License, Quit Alcohol/Drugs, Shelter, Healthcare, Childcare, Food Assistance, Record Expungement, Transportation, Vital Documents, Work Gear, Adult Education) to view the full resource guide in an embedded PDF viewer below. Only one PDF displays at a time. Clear instructions explain the 3-step process. Added static file serving for /attached_assets/ directory to serve PDFs. All 13 PDFs provided by user are stored in attached_assets/ and accessible via the page.
...
- **Authentication Bug Fix**: Fixed critical bug where all routes were requiring authentication, making public pages inaccessible. Refactored App.tsx to separate public routes (/, /programs, /quiz, /resources, /mc3, /math, /raising-the-bar, /expectation-check, /behavior-check, /express-interest, /bf-dc, /resources-guides) from protected routes (/ctw_login, /page1, /page2). Created ProtectedRoute component that only wraps protected routes.
